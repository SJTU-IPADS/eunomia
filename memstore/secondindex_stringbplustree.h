#ifndef SECONDINDEXSTRINGBPLUSTREE_H
#define SECONDINDEXSTRINGBPLUSTREE_H
...
#define ISBTREE_PROF 0
#define ISBTREE_LOCK 0
...
*/
namespace leveldb {
class SecondIndexStringBPlusTree: public SecondIndex{
...
//		uint64_t padding1[8];
	...
	class Iterator: public SecondIndex::Iterator {
	 public:
	  ...
	  Iterator(){};
	  Iterator(SecondIndexStringBPlusTree* tree);
	 ...
	 private:
	  SecondIndexStringBPlusTree* tree_;
	  LeafNode* node_;
	  uint64_t seq_;
	  int leaf_index;
	  uint64_t *link_;
...
public:	
	SecondIndexStringBPlusTree(int length) {
...
/*		if (child == NULL) {
		...
		if (d == 1) {
			...
			if (new_leaf != NULL) {
				...
				if (inner->num_keys == ISN) {										
					unsigned treshold= (ISN+1)/2;
...
//					checkConflict(new_sibling, 1);
				...
				toInsert->num_keys++;
				toInsert->keys[k] = reinterpret_cast<InnerNode*>(child)->keys[ISN-1];
...
//		if (inner->num_keys == 0) printf("inner\n");
	...
	inline LeafNode* LeafInsert(char* key, LeafNode *leaf, SecondNode** val) {
		...
		if (leaf->num_keys == ISM) {
			new_sibling = new_leaf_node();
			unsigned threshold= (ISM+1)/2;
...
//		checkConflict(leaf, 1);
	...
	SecondIndex::Iterator* GetIterator() {
		return new SecondIndexStringBPlusTree::Iterator(this);
	}
